<div class="flex flex-col h-full bg-white" wire:key="chatbox-{{ $conversationId }}">
    ...
    <div
        ...
        class="flex-1 overflow-y-auto px-4 py-5 space-y-1 scroll-smooth"
        ...
        <div class="flex {{ $msg['is_mine'] ? 'justify-end' : 'justify-start' }} items-center mb-1 group">
            ...
            {{-- OPTION MENU: Muncul di sebelah KIRI bubble hanya jika pesan milik sendiri (is_mine) --}}
            @if($msg['is_mine'])
            <div class="hidden group-hover:flex items-center gap-1 mx-2 flex-shrink-0 transition-all duration-150">
                {{-- Tombol Edit (Hanya tampil untuk tipe teks, gambar tidak bisa di-edit teksnya) --}}
                @if($msg['type'] !== 'image')
    ...
    <div class="border-t border-gray-100 px-4 py-3 bg-white flex-shrink-0">
        ...
        {{-- BAR INDIKATOR EDITING: Tampil di atas input bar jika sedang mengedit pesan --}}
        @if($editingMessageId)
        ...
        <div class="flex items-end gap-2">
            ...
            <div class="flex-1 relative">
                <textarea
                    ...
                    rows="1"
                    class="w-full resize-none border border-gray-200 rounded-2xl px-4 py-3 text-sm
                           focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent
                           transition-all bg-gray-50 focus:bg-white leading-relaxed"
                    style="min-height: 44px; max-height: 120px;"
                    oninput="this.style.height='auto'; this.style.height=Math.min(this.scrollHeight, 120)+'px'"></textarea>
            </div>

            {{-- Tombol kirim / simpan update --}}
            <button
                wire:click="sendMessage"
                wire:loading.attr="disabled"
                wire:loading.class="opacity-60"
                class="flex-shrink-0 w-11 h-11 {{ $editingMessageId ? 'bg-amber-500 hover:bg-amber-600' : 'bg-blue-600 hover:bg-blue-700' }} text-white rounded-2xl
                       flex items-center justify-center transition-all shadow-sm hover:shadow-md
                       disabled:opacity-50 disabled:cursor-not-allowed active:scale-95">
                
                {{-- Icon Check jika edit, Icon Send jika pesan baru --}}
    ...
    {{-- ═══════════════════════════════════════════════════════════
         POP-UP MODAL OVERLAY (Untuk Zoom Gambar)
    ═══════════════════════════════════════════════════════════════ --}}
