...
<div class="max-w-lg mx-auto">
    ...
    {{-- Payment Form & Dynamic Price Calculator via Alpine.js --}}
    <div x-data="{ 
        method: 'cash', 
        previewUrl: null,
        usePoints: false,
        ...
        isLoyaltyActive: {{ ($loyaltySetting && $loyaltySetting->is_active) ? 'true' : 'false' }},
        
        get discountAmount() {
            if (!this.usePoints || !this.isLoyaltyActive) return 0;
            let maxPointsNeeded = Math.ceil(this.basePrice / this.cashbackPerPoint);
            let pointsToUse = Math.min(this.userPoints, maxPointsNeeded);
            return pointsToUse * this.cashbackPerPoint;
        },
        get finalPrice() {
        ...
        <div class="bg-blue-600 rounded-2xl p-5 text-white mb-5">
            ...
            <div class="flex items-end justify-between mt-3">
                ...
                <div class="text-right">
                    ...
                    {{-- Diubah menjadi dinamis mengikuti state Alpine.js --}}
                    <p class="font-extrabold text-2xl" x-text="'Rp ' + new Intl.NumberFormat('id-ID').format(finalPrice)">Rp {{ number_format($order->total_price, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>

        {{-- Main Payment Form Box --}}
        <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            ...
            <form method="POST" action="{{ route('customer.orders.pay.store', $order) }}"
                ...
                {{-- ✨ SELEKTOR PENUKARAN POIN (SKEMA A - CASHBACK POTONGAN LANGSUNG) --}}
                @if($loyaltySetting && $loyaltySetting->is_active && auth()->user()->loyalty_points > 0)
                <div class="bg-amber-50 rounded-xl border border-amber-100 p-4 space-y-3">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs text-amber-800 font-semibold flex items-center gap-1">
                                <i data-lucide="gift" class="w-3.5 h-3.5 text-amber-600"></i> Poin Loyalitas Toko
                            </p>
                            <p class="text-sm font-black text-amber-950 mt-0.5">{{ auth()->user()->loyalty_points }} Poin Tersedia</p>
                            <p class="text-[11px] text-gray-400 mt-0.5">1 Poin = Rp {{ number_format($loyaltySetting->cashback_per_point, 0, ',', '.') }} potongan langsung</p>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="use_points" value="1" x-model="usePoints" class="sr-only peer">
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-amber-600"></div>
                        </label>
                    </div>
                    
                    {{-- Indikator Feedback Diskon --}}
                    <div x-show="usePoints && discountAmount > 0" x-transition class="bg-white border border-amber-200 rounded-lg p-2.5 text-xs text-amber-900 flex justify-between items-center font-medium">
                        <span>Potongan didapatkan:</span>
                        <span class="font-bold text-green-600">- Rp <span x-text="new Intl.NumberFormat('id-ID').format(discountAmount)"></span></span>
                    </div>
                </div>
                @endif
                    ...
                    <div class="bg-blue-50 border border-blue-100 rounded-xl p-4">
                        ...
                        <div class="space-y-1 text-sm">
                            ...
                            <div class="flex justify-between border-t border-blue-200 pt-2 mt-2">
                                ...
                                {{-- Diubah menjadi dinamis mengikuti state Alpine.js --}}
                                <span class="font-bold text-blue-700" x-text="'Rp ' + new Intl.NumberFormat('id-ID').format(finalPrice)">Rp {{ number_format($order->total_price,0,',','.') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="method === 'qris'" x-transition>
                    ...
                    <div class="bg-purple-50 border border-purple-100 rounded-xl p-4 text-center">
                        ...
                        @endif
                        {{-- Diubah menjadi dinamis mengikuti state Alpine.js --}}
                        <p class="text-sm font-bold text-purple-700 mt-2" x-text="'Rp ' + new Intl.NumberFormat('id-ID').format(finalPrice)">Rp {{ number_format($order->total_price,0,',','.') }}</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="notes" value="{{ old('notes') }}" placeholder="No. transaksi, keterangan, dll."
                ...
                <button type="submit"
                    ...
                    <span x-text="method === 'cash' ? 'Konfirmasi Bayar di Toko' : 'Kirim Bukti Pembayaran'">Kirim Pembayaran</span>
                </button>
            </form>
        </div>
    </div>
