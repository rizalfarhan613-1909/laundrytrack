...
<div class="max-w-2xl mx-auto space-y-4">
    ...
    {{-- Tombol Hubungi Admin Laundry --}}
    <div class="mt-4">
        {{-- Asumsi ID user admin/pemilik laundry adalah 1, atau sesuaikan dengan relasi di aplikasi kamu --}}
        <form action="{{ route('chat.start') }}" method="POST">
            @csrf
            <input type="hidden" name="with_user_id" value="1"> {{-- Target ID Admin/Pemilik --}}
            <input type="hidden" name="order_id" value="{{ $order->id }}"> {{-- Keterangan order saat ini --}}
        ...
        <div class="flex items-center justify-between p-4 rounded-xl
            ...
            <div class="flex items-center gap-3">
                <i data-lucide="{{ $pay->isVerified() ? 'check-circle' : ($pay->status==='rejected' ? 'x-circle' : 'clock') }}"
                    class="w-5 h-5 {{ $pay->isVerified() ? 'text-green-500' : ($pay->status==='rejected' ? 'text-red-500' : 'text-amber-500') }}"></i>
                <div>
