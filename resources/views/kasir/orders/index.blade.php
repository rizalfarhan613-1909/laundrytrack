    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-50 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
            ...
            <form method="GET" action="{{ route('kasir.dashboard') }}" class="flex gap-2">
                ...
                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-blue-700">Cari</button>
            </form>
        </div>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors group" x-data="{ weightEdit: false }">
                        ...
                        <td class="px-4 py-4">
                            ...
                            @if(isset($order->is_halal_service) && $order->is_halal_service)
                            <div class="mt-1.5">
                                @if($order->halal_status === 'pending_thaharah' && $order->has_najis)
                                ...
                                @elseif($order->halal_status === 'thaharah_completed')
                                ...
                                @elseif($order->halal_status === 'main_wash')
                                ...
                                @elseif(!$order->has_najis && $order->halal_status !== 'main_wash')
                                <span class="text-[11px] bg-indigo-50 text-indigo-700 border border-indigo-200 px-1.5 py-0.5 rounded font-medium flex items-center gap-1 w-max">
                                    ✨ Bebas Najis (Suci)
                                </span>
                                @endif
                            </div>
                        ...
                        <td class="px-4 py-4">
                            ...
                            <div>
                                ...
                                <form method="POST" action="{{ route('kasir.payments.verify', $pay) }}" class="mt-1">
                                    ...
                                    <button type="submit" class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded hover:bg-green-200">Verifikasi</button>
                                </form>
                                @endif
                            </div>
                            @else
                        ...
                        <td class="px-4 py-4">
                            <div class="flex flex-col gap-1.5 items-start">
                                ...
                                @if(isset($order->is_halal_service) && $order->is_halal_service)
                                    ...
                                    @elseif($order->halal_status === 'thaharah_completed' || (!$order->has_najis && $order->halal_status !== 'main_wash'))
                                ...
                                @if($order->getNextStatus())
                                    {{-- Jika layanan halal & belum masuk mesin cuci utama, kunci tombol status utama --}}
                                    @if(isset($order->is_halal_service) && $order->is_halal_service && $order->halal_status !== 'main_wash')
                                    <span class="text-[11px] text-gray-400 italic block mt-1">Selesaikan SOP Halal</span>
                                    @else
                                    <form action="{{ route('kasir.orders.advance', $order->id) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        {{-- Tombol Maju Status: Background Biru Pudar (bg-blue-100) --}}
                                        <button type="submit" class="text-xs bg-blue-100 text-blue-700 hover:bg-blue-200 px-3 py-1.5 rounded-lg font-medium transition-colors whitespace-nowrap flex items-center gap-1">
                                            Maju Status <i data-lucide="chevron-right" class="w-3 h-3"></i>
                                        </button>
                                    </form>
                                    @endif
                                @else
